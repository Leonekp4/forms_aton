                        ...
                        <div class="item-main">
                            ...
                            <div class="status-selector">
                                <label class="status-btn conforme">
                                    <input type="radio" :name="'status-' + index" value="C"
                                        v-model="formData.respostas[index].status">
                                    <span class="circle">C</span>
                                </label>

                                <label class="status-btn nao-conforme">
                                    <input type="radio" :name="'status-' + index" value="NC"
                                        v-model="formData.respostas[index].status">
                            ...
                            <div v-if="formData.respostas[index].status === 'NC'" class="nc-details">
                                ...
                                <div class="nc-options">
                                    <label v-for="falha in item.opcoesFalha" :key="falha.id" class="checkbox-label">
                                        <input type="checkbox" :value="falha.label"
                                            v-model="formData.respostas[index].falhas">
                                        {{ falha.label }}
                                    </label>
                                </div>
                            </div>
...
const handleSubmit = async () => {
    try {
        ...
        if (!matricula || !nome || !turno || !celula || !maquina) {
            notificacao.value.mostrarAlerta("warning", "warning", "Aviso", "Por fazor, preencha todos os dados do Usuário")
            return
        }
        ...
        if (incompleto) {
            notificacao.value.mostrarAlerta("warning", "warning", "Aviso", "Por favor, responda todos os itens do checklist antes de enviar.")
            return
        }
        ...
        if (semFalhas) {
            notificacao.value.mostrarAlerta("warning", "warning", "Aviso", "Para intens 'Não Conforme', selecione pelo menos uma opção")
            return
        }
        ...
        console.error("Erro ao savlar checklist");
        console.error(error);
        notificacao.value.mostrarAlerta("warning", "warning", "Aviso", "Erro ao savlar checklist.")
...
async function buscaDadosColaborador() {
    ...
    if (!matricula || matricula === 0 || matricula === '') {
        if (notificacao.value) {
            notificacao.value.mostrarAlerta("warning", "warning", "Aviso", "Digite a matrícula para buscar seus dados.")
        }
        return
    }
    ...
    if (matricula.length < 7) {
        if (notificacao.value) {
            notificacao.value.mostrarAlerta("warning", "warning", "Aviso", "Digite uma mnatricula válida. Lembre-se de colocar o 30 na frente.")
        }
        return;
    }
    ...
    notificacao.value.mostrarAlerta("success", "done_outlined", "Sucesso!", mensagemApi || "Dados colaborador resgatado com sucesso!")

    const turno = dadosColaborador.nome_setor[dadosColaborador.nome_setor.length - 1]

    formData.usuario.nome = dadosColaborador.nome
    formData.usuario.gerente = dadosColaborador.gerente

    // Verifica se é turno A ou B
    const tunoAB = turno.toUpperCase() === 'A' || turno.toUpperCase() === 'B'
    formData.usuario.turno = tunoAB ? turno : 'C'
}
